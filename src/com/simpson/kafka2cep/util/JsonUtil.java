    ...
    static public void main( String []aArgv )
    {
        ...
        if( aArgv.length != 2 )
        {
            System.out.println( "Usage : \n" +
                    "  java JsonUtil [file name] [data item number]\n" );
             JsonUtil.getLines( mExample );
        }
        else
        {
            try
            {
                ...
                if( sFile.exists() == true )
                {
                    ...
                    if( sFileReader.read( sLines ) > 0 )
                    {
                        ...
                        Pattern mLinePattern = Pattern.compile( JsonUtil.mPattern4JSONLine );
                        ...
                        while( mLineMatcher.find() )
                        {
                            ...
                            if( sLineField.length() > 0 )
                            {
                                ...
                                sFields = JsonUtil.getValues( sLineField , sItemCnt );
                                if( sFields != null )
                                {
